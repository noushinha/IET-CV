...
frames = []
rng = 0
base_dir = "/media/Data/Entropy/Data/Features/RML/20/18/Combined/" # "/media/Data/Entropy/Data/Windows/RML/20/18/Angry/"
base_dir2 = "/media/Data/Entropy/Data/Kmeans/RML/20/18/"  # "/media/Data/Entropy/Data/Kmeans/RML/20/18/Angry/"
for i in range(0,1):
    ...
    for p in range(0,13):
        features = pd.read_csv(os.path.join(base_dir, class_list[i], class_list2[i] + '_' + str(p) + '.csv'), header=None)
        ...
        for j in range(0,rng1):
            ...
            if k in indices.iloc[p][:]:
                label = 0 #i # selected spectrograms
            else:
                label = 1 #i+6 # non-selected spectrograms
            frames.append([features[j][:], label])
            # frames.append([features.iloc[j][:], label])
