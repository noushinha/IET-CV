...
base_dir   = "/media/Data/IEEE Transaction on Affective Computing/Data/" + Dataset + "/AudioBase/Video/Train_Test"
res_dir    = "/media/Data/IEEE Transaction on Affective Computing/Result/" + Dataset + "/AudioBase/Video/"
...
def plot_folds_barchart(t_data, v_data, fold_number, fold_dir):
    ...
    autolabel(ax, rects1)
    autolabel(ax, rects2)
    filename = os.path.join(fold_dir, Dataset + "_Face_Training_Validation_Fold_" + str(fold_number) + "_Distribution_" + str(epoch) + "_Epoch.eps")
    plt.savefig(filename, format='eps', dpi=1000, bbox_inches="tight")
    plt.close(barfig)
...
for train, vald in kfold.split(train_data, train_labels):
    ...
    # avg_validation_accuracy += np.mean(val_acc)

    idx += 1

# print(avg_validation_accuracy /k)
...
CF_NonNormalized_filename = os.path.join(eps_dir, Dataset + "_Face_NonNormalized_" + str(epoch) + "_Epoch_" + data_type +".eps")
...
CF_Normalized_filename = os.path.join(eps_dir, Dataset + "_Face_Normalized_" + str(epoch) + "_Epoch_" + data_type +".eps")
...
shutil.copyfile(os.path.join(file_path, Dataset + "_AudioBase_Video_CNN_KFold.py"), os.path.join(cur_res_dir, Dataset + "_AudioBase_Video_CNN_KFold.txt"))
plt.show(block=True)