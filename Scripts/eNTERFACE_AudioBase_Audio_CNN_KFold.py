...
file_path ="/media/Data/IEEE Transaction on Affective Computing/Code/"
base_dir   = "/media/Data/IEEE Transaction on Affective Computing/Data/" + Dataset + "/AudioBase/Audio/Train_Test"
res_dir    = "/media/Data/IEEE Transaction on Affective Computing/Result/" + Dataset + "/AudioBase/Audio/"
...
#load data
train_data = np.load(os.path.join(base_dir, "Spec_train_data_" + data_type +".npy"))
test_data  = np.load(os.path.join(base_dir, "Spec_test_data_" + data_type +".npy"))

print(train_data.shape)

train_labels = np.load(os.path.join(base_dir, "Spec_train_label_" + data_type +".npy"))
test_labels  = np.load(os.path.join(base_dir, "Spec_test_label_" + data_type +".npy"))
...
def normalizing_data(train_data, test_data):
    mean = train_data.mean(axis=0)
    train_data2 = train_data - mean
...
def plot_confusion_matrix(cm, classes,
    ...
    if normalize:
        CF_NonNormalized_filename = os.path.join(eps_dir, Dataset + "_Spec_NonNormalized_" + str(epoch) + "_Epoch_" + data_type + ".eps")
        plt.savefig(CF_NonNormalized_filename, format='eps', dpi=500, bbox_inches="tight")
    else:
        CF_Normalized_filename = os.path.join(eps_dir, Dataset + "_Spec_Normalized_" + str(epoch) + "_Epoch_" + data_type + ".eps")
...
def plot_train_vs_vald(train_points, vald_points, isLoss=False):
    ...
    plt.legend()
    filename = os.path.join(eps_dir, Dataset + "_Spec_Training_Validation_" + plot_label + "_" + str(
        epoch) + "_Epoch.eps")
    plt.savefig(filename, format='eps', dpi=1000, bbox_inches="tight")


def plot_folds_accuracy(model_history):
    ...
    filename = os.path.join(eps_dir, Dataset + "_Spec_Folds_Training_Validation_Accuracy_" + str(
        epoch) + "_Epoch.eps")
    plt.savefig(filename, format='eps', dpi=1000, bbox_inches="tight")


def plot_folds_loss(model_history):
    ...
    plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.05), shadow=False, ncol=5)
    filename = os.path.join(eps_dir, Dataset + "_Spec_Folds_Training_Validation_Loss_" + str(
        epoch) + "_Epoch.eps")
    plt.savefig(filename, format='eps', dpi=1000, bbox_inches="tight")
...
def plot_folds_barchart(t_data, v_data, fold_number, fold_dir):
    ...
    autolabel(ax, rects1)
    autolabel(ax, rects2)
    filename = os.path.join(fold_dir, Dataset + "_Spec_Training_Validation_Fold_" + str(fold_number) + "_Distribution_" + str(epoch) + "_Epoch.eps")
    plt.savefig(filename, format='eps', dpi=1000, bbox_inches="tight")
    plt.close(barfig)
...
def plot_ROC(y_test, y_score):
    ...
    filename = os.path.join(eps_dir, Dataset + "_Spec_Micro_Macro_Avg_ROC_Curve_" + str(epoch) + "_Epoch.eps")
    plt.savefig(filename, format='eps', dpi=1000, bbox_inches="tight")
...
def plot_lr(lr_points):
    ...
    plt.legend()
    filename = os.path.join(eps_dir, Dataset + "_Spec_Learning_Rate_" + str(epoch) + "_Epoch.eps")
    plt.savefig(filename, format='eps', dpi=1000, bbox_inches="tight")
...
def create_model():
    ...
    model.add(layers.Conv3D(filters=32, kernel_size=(3, 3, 3), strides=(1, 1, 1),
                            input_shape=(9, image_width, image_height, 3),
    ...
    model.add(layers.MaxPooling3D(pool_size=(3, 3, 3), strides=(2, 2, 2), padding='VALID', name="maxpool3d_1"))
    ...
    model.add(layers.Conv3D(filters=64, kernel_size=(3, 3, 3), strides=(1, 1, 1),
                            ...
                            kernel_regularizer=l2(0.0), bias_initializer='random_uniform', padding='SAME'))
    model.add(layers.MaxPooling3D(pool_size=(3, 5, 5), strides=(2, 2, 2), padding='VALID', name="maxpool3d_2"))
    ...
    #                         kernel_regularizer=l2(0.0), bias_initializer='random_uniform', padding='SAME'))
    model.add(layers.MaxPooling3D(pool_size=(1, 7, 7), strides=(2, 2, 2), padding='VALID', name="maxpool3d_3"))
    #
    # model.add(layers.Conv3D(filters=128, kernel_size=(3, 3, 3), strides=(1, 1, 1),
    #                         kernel_initializer='random_uniform', activation="relu", name="conv3d_4",
    #                         kernel_regularizer=l2(0.0), bias_initializer='random_uniform', padding='SAME'))
    # model.add(layers.MaxPooling3D(pool_size=(1, 3, 1), strides=(1, 2, 1), padding='VALID', name="maxpool3d_4"))
    ...
    # set the optimizer
    optimizer = set_optimizer(type="Adam")
...
with open(os.path.join(cur_res_dir, Dataset + "_Spec_model.json"), "w") as json_file:
...
with open(os.path.join(cur_res_dir, Dataset + "_Spec_HyperParameters.txt"), "w") as text_file:
    text_file.write(HyperParameter_Setting)

print(HyperParameter_Setting)
shutil.copyfile(os.path.join(file_path, Dataset + "_VideoBase_Audio_CNN_KFold.py"), os.path.join(cur_res_dir, Dataset + "_VideoBase_Audio_CNN_KFold.txt"))
plt.show(block=True)