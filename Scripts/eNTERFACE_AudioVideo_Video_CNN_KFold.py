...
base_dir   = "/media/Data/IEEE Transaction on Affective Computing/Data/" + Dataset + "/AudioVideo/Video/Train_Test"
res_dir    = "/media/Data/IEEE Transaction on Affective Computing/Result/" + Dataset + "/AudioVideo/Video/"
...
#load data
train_data = np.load(os.path.join(base_dir, "Face_Spec_train_data_" + data_type +".npy"))
test_data  = np.load(os.path.join(base_dir, "Face_Spec_test_data_" + data_type +".npy"))

print(train_data.shape)

train_labels = np.load(os.path.join(base_dir, "Face_Spec_train_label_" + data_type +".npy"))
test_labels  = np.load(os.path.join(base_dir, "Face_Spec_test_label_" + data_type +".npy"))
...
def plot_train_vs_vald(train_points, vald_points, isLoss=False):
    ...
    plt.legend()
    filename = os.path.join(eps_dir, Dataset + "_Face_Spec_Training_Validation_" + plot_label + "_" + str(
        epoch) + "_Epoch.eps")
...
def plot_folds_accuracy(model_history):
    ...
    filename = os.path.join(eps_dir, Dataset + "_Face_Spec_Folds_Training_Validation_Accuracy_" + str(
        epoch) + "_Epoch.eps")
    plt.savefig(filename, format='eps', dpi=1000, bbox_inches="tight")
...
def plot_folds_loss(model_history):
    ...
    plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.05), shadow=False, ncol=5)
    filename = os.path.join(eps_dir, Dataset + "_Face_Spec_Folds_Training_Validation_Loss_" + str(
        epoch) + "_Epoch.eps")
    plt.savefig(filename, format='eps', dpi=1000, bbox_inches="tight")
...
def plot_folds_barchart(t_data, v_data, fold_number, fold_dir):
    ...
    autolabel(ax, rects1)
    autolabel(ax, rects2)
    filename = os.path.join(fold_dir, Dataset + "_Face_Spec_Training_Validation_Fold_" + str(fold_number) + "_Distribution_" + str(epoch) + "_Epoch.eps")
    plt.savefig(filename, format='eps', dpi=1000, bbox_inches="tight")
    plt.close(barfig)
...
def plot_ROC(y_test, y_score):
    ...
    filename = os.path.join(eps_dir, Dataset + "_Face_Spec_Micro_Macro_Avg_ROC_Curve_" + str(epoch) + "_Epoch.eps")
    plt.savefig(filename, format='eps', dpi=1000, bbox_inches="tight")
...
def plot_lr(lr_points):
    ...
    plt.legend()
    filename = os.path.join(eps_dir, Dataset + "_Face_Spec_Learning_Rate_" + str(epoch) + "_Epoch.eps")
    plt.savefig(filename, format='eps', dpi=1000, bbox_inches="tight")
...
def create_model():
    ...
    model.add(layers.Conv3D(filters=16, kernel_size=(3, 3, 3), strides=(1, 1, 1),
                            input_shape=(18, image_width, image_height, 3),
...
with open(os.path.join(cur_res_dir, Dataset + "_Face_Spec_model.json"), "w") as json_file:
...
CF_NonNormalized_filename = os.path.join(eps_dir, Dataset + "_Face_Spec_NonNormalized_" + str(epoch) + "_Epoch_" + data_type +".eps")
...
CF_Normalized_filename = os.path.join(eps_dir, Dataset + "_Face_Spec_Normalized_" + str(epoch) + "_Epoch_" + data_type +".eps")
...
with open(os.path.join(cur_res_dir, Dataset + "_Face_Spec_Spec_HyperParameters.txt"), "w") as text_file:
    text_file.write(HyperParameter_Setting)

print(HyperParameter_Setting)
shutil.copyfile(os.path.join(file_path, Dataset + "_AudioVideo_Video_CNN_KFold.py"), os.path.join(cur_res_dir, Dataset + "_AudioVideo_Video_CNN_KFold.txt"))
plt.show(block=True)